...
func (g *gen) program(prog *ast.Program) {
	...
	g.line("package main")
	g.line("")
	base := []string{"fmt", "strconv"}
	if len(g.records) > 0 {
		base = append(base, "reflect", "strings")
	}
	imports := append(base, chunkImports(chunks)...)
	sort.Strings(imports)
	...
	// print() renders it, preceded once by the shared rendering runtime.
	if len(g.records) > 0 {
		g.line("")
		g.raw(recordSupport)
	}
...
// recordSupport is the shared rendering runtime emitted once when a program
// declares any record. _ink_render carries a depth counter through nested
// records *and lists of records*, so a cyclic record (only constructible via
// a list field, since there is no nil literal) collapses to Name(...) at the
// cap instead of overflowing the stack. It uses reflection to descend into
// slices of any element type without a per-type helper.
const recordSupport = `type _ink_recordStringer interface {
	_ink_str(int) string
}

func _ink_render(v any, depth int) string {
	if s, ok := v.(_ink_recordStringer); ok {
		return s._ink_str(depth)
	}
	rv := reflect.ValueOf(v)
	if rv.Kind() == reflect.Slice {
		parts := make([]string, rv.Len())
		for i := 0; i < rv.Len(); i++ {
			parts[i] = _ink_render(rv.Index(i).Interface(), depth)
		}
...
// as Name(field: value, ...). Every field is rendered through _ink_render,
// which threads the depth counter through nested records and record lists, so
// beyond a small depth (and for any cycle) nested records collapse to
// Name(...).
func (g *gen) recordDecl(rec *ast.RecordDecl) {
	...
	var parts []string
	for _, f := range rec.Fields {
		field := recv + "." + sanitize(f.Name)
		parts = append(parts, fmt.Sprintf("%q + _ink_render(%s, _ink_depth+1)", f.Name+": ", field))
	}
